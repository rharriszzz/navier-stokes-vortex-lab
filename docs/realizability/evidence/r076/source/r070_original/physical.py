...
import launch_guard as launch_guard
import numpy as np
import presolve
...
def record_compatibility(vector, null_vectors, record, checkpoint, key, stage,
                         apply=True, nullspace=None):
    """Measure on a copy; preserve the raw candidate even on refusal."""
    raw_hash = presolve.vector_sha(vector)
    rhs_norm = float(vector.norm())
    products = [float(null.dot(vector)) for null in null_vectors]
    record[key] = dict(raw_sha256=raw_hash, rhs_norm=rhs_norm,
        pressure_constant_products_before=products, tolerance=256*np.finfo(float).eps*rhs_norm)
    checkpoint(stage)
    projected = vector.copy()
    if nullspace is None:
        for null in null_vectors:
            projected.axpy(-null.dot(projected), null)
    else:
        nullspace.remove(projected)
    removed = vector.copy()
    ...
    data = dict(pressure_constant_products_before=products,
                ...
                tolerance=256*np.finfo(float).eps*rhs_norm, raw_sha256=raw_hash,
                proposed_sha256=presolve.vector_sha(projected),
                raw_unchanged_after_measurement=presolve.vector_sha(vector)==raw_hash)
    removed.destroy()
    assert data['raw_unchanged_after_measurement']
    data['accepted'] = bool(data['removed_norm'] <= data['tolerance'])
    data['removal_applied'] = False
    record[key] = data
    checkpoint(stage)
    ...
    if apply:
        projected.copy(vector)
        data['removal_applied'] = True
    data['accepted_sha256'] = presolve.vector_sha(vector)
    projected.destroy()
    checkpoint(stage)
    return data


def record_returned_solve(record, checkpoint, stage, factor_counts, reason):
    """Validate convergence/events at the first point after primary return."""
    expected_solves = record.get('matrix_solves', 0) + 1
    launch_guard.record_solve(record, stage, int(reason), factor_counts(), expected_solves)
    record['factor_counts_at_return'] = factor_counts()
    checkpoint(stage)

def identities():
    return {name:support.sha(HERE/name) for name in ('physical.py','toy_runner.py','kernels.py','disk.py','presolve.py')}
...
def calculate(output):
    ...
    record=dict(status='partial',stage='imports',campaign_ready=False,physical_gate_passed=False,
        ...
        attempted_primary_solves=0,returned_primary_solves=0,
        attempted_correction_solves=0,returned_correction_solves=0,completed_rhs_assemblies=0,
    ...
    try:
        ...
        def capture(*args,**kwargs):
            ...
            def candidate_factory(name, local):
                projection, load = loads[name]
                current_spaces = local['spaces']
                normal_real = fem.Function(current_spaces[0])
                normal_imag = fem.Function(current_spaces[2])
                normal_real.x.array[:] = projection.real
                normal_imag.x.array[:] = projection.imag
                real_bcs, _, _ = base._velocity_boundary_conditions(current_spaces[0], tags,
                    normal_real, fem.Function(current_spaces[0]))
                imag_bcs, _, _ = base._velocity_boundary_conditions(current_spaces[2], tags,
                    normal_imag, fem.Function(current_spaces[2]))
                return dict(bcs=real_bcs + imag_bcs, normal_real=normal_real,
                    normal_imag=normal_imag, projection=projection,
                    block_loads=[load.real, np.zeros(nq), load.imag, np.zeros(nq)],
                    flux_ratios=record['loads'][name]['flux_ratios'])
            result=presolve.call_with_barrier(original,captured,args,kwargs,expected,
                candidate_factory,prepared,record,checkpoint)
            record_returned_solve(record,checkpoint,'P_solver_returned',factor_counts,
                captured['solver'].getConvergedReason())
            return result
        ...
        def correction(name,rhs,solution):
            ...
            record['attempted_correction_solves']+=1
            checkpoint(name+'_correction_solve')
            ksp.solve(projected,dx)
            launch_guard.record_correction(record, name+'_correction_solver_returned',
                int(ksp.getConvergedReason()), counts(), record.get('matrix_solves', 0) + 1)
            checkpoint(name+'_correction_solver_returned')
        ...
        def outputs(name,packs):
            ...
            corrected_magnitude_error=abs(abs(gc)-abs(reference))/abs(reference)
            corrected_phase=None if gc==0 else abs(float(np.angle(gc/reference,deg=True)))
            row['corrected_comparison']=dict(complex_error_per_m=pair(gc-reference),
                absolute_complex_error_per_m=abs(gc-reference),
                relative_complex_error=abs(gc-reference)/abs(reference),
                relative_magnitude_error=corrected_magnitude_error,
                wrapped_phase_error_deg=corrected_phase,
                magnitude_passed=corrected_magnitude_error<.05,
                phase_passed=corrected_phase is not None and corrected_phase<5.)
            row['comparison']['complex_error_per_m']=pair(g-reference)
        ...
        for name,candidate in prepared.items():
            ...
            normal_real,normal_imag=candidate['normal_real'],candidate['normal_imag']
            new_grouped=candidate['grouped_bcs']
            rhs=candidate['rhs']
            accepted=record['candidates'][name]['pressure_compatibility']['accepted_sha256']
            assert presolve.vector_sha(rhs)==accepted
            assert matrix_digest()==record['pre_solve_operator']['digest']
            assert K.stateGet()==record['pre_solve_operator']['state']
            record['candidates'][name]['retained_rhs_reused_without_assembly']=True
        ...
        load_step=abs(complex(*record['outputs']['A_96']['primary']['gain'])-
                      complex(*record['outputs']['A_64']['primary']['gain']))
...
if __name__=='__main__':
    ...
    if args.child:
        raise SystemExit('physical child execution disabled pending reviewed launch contract')
    if args.extra_toys:
        ...
        raise SystemExit('legacy physical launch path disabled; use guarded parent only')
