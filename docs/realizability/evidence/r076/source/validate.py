...
def drive(tree_factory, *, cleanup=None, policy=None, sink=None):
    clock = Clock()
    ...
    result = core.supervise(policy=policy or core.Policy(1536, 1.0),
        ...
        cleanup=cleanup or (lambda limit: {"confirmed": True, "survivors": [], "errors": []}),
        report_sink=sink)
...
def test_rss_boundary_and_host_stop():
    ...
    result = core.supervise(policy=core.Policy(1536, 1),
        ...
        cleanup=lambda _: {"confirmed": True, "survivors": [], "errors": []})
...
def test_host_timeout_and_clock_regression():
    ...
    result = core.supervise(policy=core.Policy(1536, 2),
        ...
        host_request=lambda *_: None, clock=clock.now, wait=clock.wait,
        cleanup=lambda _: {"confirmed": False, "survivors": None, "errors": []})
...
def test_disabled_r070_and_completion_binding():
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        good_child = {"status": "complete", "stage": "fixture", "evidence_complete": True,
...
def test_continuity_and_source_bindings():
    ...
    docs = [repo / "SESSION_HANDOFF.md",
            repo / "docs/realizability/B2_MONITOR_ADAPTER_REVIEW.md",
    ...
    assert "monitor implementation is complete" in handoff
    assert request_log.count("## R076 —") == 1
    assert sessions.count("## R076 —") == 1
    r076_session = sessions.split("## R076 —", 1)[1]
    assert sessions.count("STARTED | 2026-09-21 04:52:05 UTC") == 1
    assert r076_session.count("COMPLETED | 2026-09-21") == 1
    ...
    for json_path in evidence_root.rglob("*.json"):
        ...
        json_count += 1
    ledger = json.loads((repo / "docs/realizability/evidence/r076/attempt_ledger.json").read_text())
    assert ledger["cumulative_elapsed_seconds"] <= 120
    assert ledger["maximum_measured_child_rss_bytes"] <= 256 * 1024 * 1024
    assert len(ledger["attempts"]) >= 6
    assert all(row.get("state") == "COMPLETED" for row in ledger["attempts"][:-1])
    assert ledger["attempts"][-1]["state"] == "STARTED"
    assert set(ledger["attempts"][-1]["command"]) == {"runner", "validator", "working_directory"}
    reconciled_commands = {item["attempt"] for item in ledger["recording_reconciliations"]}
    assert all("command" in item or item["attempt"] in reconciled_commands
    ...
    validation = {"schema": 1, "status": "passed", "checked_documents": [
        str(path.relative_to(repo)) for path in docs], "local_links": link_count,
        "source_hashes": len(manifest["source_sha256"]), "continuity": "R076 request/session/handoff consistent",
        "attempts_including_current": len(ledger["attempts"]),
        "completed_attempts_before_current": len(ledger["attempts"]) - 1,
        "cumulative_seconds_before_current": ledger["cumulative_elapsed_seconds"],
        "maximum_child_rss_bytes": ledger["maximum_measured_child_rss_bytes"],
        "reconstructed_prior_commands": len(reconciled_commands),
        "finite_json_files": json_count}
    evidence = repo / "docs/realizability/evidence/r076/documentation_validation.json"
...
def main():
    ...
    attempt_number = int(os.environ.get("R076_ATTEMPT", "1"))
