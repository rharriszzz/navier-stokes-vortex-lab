...
def finalize_child(report: dict, watch: dict, child_report: dict | None, *,
    ...
    accepted = (
        watch.get("schema") == 3 and child_report.get("schema") == 2 and
...
def _cleanup_ok(value) -> bool:
    ...
    actions_ok = (isinstance(actions, list) and all(isinstance(item, str) and item for item in actions))
