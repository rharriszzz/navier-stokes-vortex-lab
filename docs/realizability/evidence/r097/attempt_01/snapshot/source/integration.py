"""R097 fixture-only composition. No OS acquisition, process launch or physical API.
...
import r070_legacy
from certificate import Admission, validate_monitor, validate_outer
...
class Owner:
    def __init__(self, ownership: Ownership, admission: Admission):
        require(type(ownership) is Ownership, 'invalid ownership schema')
        ownership.validate()
        require(type(admission) is Admission, "immutable admission required")
        admission.validate(ownership)
...
def finalize(owner: Owner, *, clock: Clock, outer_deadline: float,
    ...
    candidate = {'schema': SCHEMA, 'status': 'candidate', 'ownership': owner.ownership.record(),
                 'admission_sha256': owner.admission.sha256(), 'history': list(owner.history), 'released': owner.released,
    ...
    receipt = {'schema': SCHEMA, 'status': 'accepted' if not owner.errors else 'refused',
               'admission_sha256': owner.admission.sha256(), 'candidate_sha256': saved_hash, 'observed_at': end,
...
def supervise(owner: Owner, *, policy: Policy, outer_deadline: float, now,
    ...
    try:
        policy.validate()
        owner.admission.validate(owner.ownership)
        require(digest(asdict(policy)) == digest(asdict(owner.admission.policy)) and
                outer_deadline == owner.admission.recorder_deadline, "admitted policy/deadline mismatch")
        _number('outer deadline', outer_deadline, positive=True)
        ...
        started = clock.read()
        require(started >= owner.admission.reserved_at, "clock predates admission")
        phase = started
        next_tree = started
        next_host = started
        host = None
        ...
        while True:
            ...
            if owner.released and current >= next_tree:
                ...
                report['samples'].append({'sequence': seq, 'start': begin, 'end': end,
                                          ...
                                          'root_alive': reading.root_alive,
                                          'root_returncode': reading.root_returncode,
                                          'member_count': len(reading.members),
                                          'membership_complete': reading.membership_complete})
...
def validate_candidate(value, owner, admission):
    """Apply independently to memory and durable JSON, before comparing bytes."""
    require(type(value) is dict and set(value) == {'schema', 'status', 'ownership',
        'history', 'released', 'requested', 'cleanup', 'errors', 'cleanup_started',
        'cleanup_observed', 'cleanup_deadline', 'admission_sha256'}, 'candidate fields')
    require(type(value['schema']) is int and value['schema'] == SCHEMA and
        value['status'] == 'candidate' and type(value['errors']) is list and
        not value['errors'] and value['released'] is True, 'candidate schema/status')
    require(digest(value['ownership']) == digest(owner.record()) and
        value['admission_sha256'] == admission.sha256(), 'candidate admission/owner')
    require(type(value['history']) is list and value['history'] ==
        ['RESERVED', 'OWNED_HELD', 'READY', 'RUNNING', 'STOPPING', 'FINALIZED'] and
        type(value['requested']) is dict and
        value['requested'] == {n:'stop_requested' for n in DOMAINS}, 'candidate lifecycle')
    start = _number('candidate cleanup start', value['cleanup_started'])
    observed = _number('candidate cleanup observation', value['cleanup_observed'])
    deadline = _number('candidate cleanup deadline', value['cleanup_deadline'])
    require(admission.reserved_at <= start < admission.policy.deadline and
        start <= observed <= deadline == min(start + 5.0, admission.recorder_deadline),
        'candidate absolute cleanup bounds')
    require(type(value['cleanup']) is dict and set(value['cleanup']) == set(DOMAINS),
        'candidate cleanup domains')
    for name in DOMAINS:
        domain = getattr(owner, name)
        require(domain is not None, 'candidate missing owned domain')
        cleanup_observation(domain, name, value['cleanup'][name])


def accept_sentinel(owner: Ownership, monitor: dict, child: dict, *, source_root,
                    source_manifest_path, persisted_candidate: dict, admission: Admission,
                    outer_receipt, terminal_witness) -> dict:
    """R070 acceptance adapter; the child cannot nominate its cleanup owner.

    Reuses archived R070 semantic checks only after the composed v5 proof has
    ...
    try:
        owner.validate()
        require(type(admission) is Admission, "immutable admission required")
        admission.validate(owner)
        validate_monitor(monitor, child, admission, COUNTS)
        ...
        validate_candidate(candidate, owner, admission)
        validate_candidate(persisted_candidate, owner, admission)
        ...
        require(type(receipt) is dict and set(receipt) == {'schema', 'status',
            'candidate_sha256', 'observed_at', 'errors', 'admission_sha256'}, 'receipt fields')
        ...
        require(type(persisted_candidate) is dict and
                type(persisted_candidate.get('schema')) is int and
                persisted_candidate['schema'] == SCHEMA and
                finite_json(candidate) == finite_json(persisted_candidate) and
                receipt['candidate_sha256'] == digest(persisted_candidate),
        ...
        require(admission.reserved_at <= start < admission.policy.deadline and
                start <= observed <= received <= deadline and
                deadline == min(start + 5.0, admission.recorder_deadline), 'cleanup time bounds')
        require(candidate['admission_sha256'] == receipt['admission_sha256'] ==
                admission.sha256(), 'admission binding differs')
        validate_outer(outer_receipt, terminal_witness, admission, owner, monitor, child, persisted_candidate)
        ...
        return {'schema': SCHEMA, 'status': 'complete', 'counts': deepcopy(child['counts']),
                ...
                'outer_receipt_sha256': digest(outer_receipt), 'scope': 'fake_only',
