...
def check(name, fn):
    ...
    progress = {"schema": 1, "attempt": attempt_number, "state": "RUNNING",
        "checks_completed": len(checks), "checks": checks}
    core.durable_checkpoint(ROOT.parent / f"progress_attempt_{attempt_number:02d}.json", progress)
...
def test_attempt_accounting_refuses_open_unknown_failed_and_exhausted():
    ...
    reconciliation = {"attempt": 1, "classification": "resolved_fixture_assertion",
        ...
        "reason": "A correct terminal protocol latch invalidated the fixture's reuse assumption.",
        "failed_source_sha256": source_map, "corrected_source_sha256": source_map}
    assert prior_elapsed([failed, {**good, "attempt": 2}], reconciliations=[reconciliation]) == 4.0
    raises(ValueError, lambda: prior_elapsed([failed, {**good, "attempt": 2}], reconciliations=[
        {**reconciliation, "failed_source_sha256": {}}]))
...
def test_host_protocol_success_and_sequence():
    ...
    raises(core.MonitorError, lambda: protocol.reply(reply(2), 0.7))
...
def test_continuity_and_source_bindings():
    ...
    for relative, expected in manifest["source_sha256"].items():
        actual = hashlib.sha256((evidence_root / "source" / relative).read_bytes()).hexdigest()
        assert actual == expected, relative
    active_number = int(os.environ.get("R084_ATTEMPT", "1"))
    active_attempt = json.loads((evidence_root / f"attempt_{active_number:02d}.json").read_text())
...
def main():
    ...
    for name, function in checks_to_run:
        check(name, function)
    attempt_number = int(os.environ.get("R084_ATTEMPT", "1"))
    core.durable_checkpoint(ROOT.parent / f"progress_attempt_{attempt_number:02d}.json",
        {"schema": 1, "attempt": attempt_number, "state": "COMPLETED",
         "checks_completed": len(checks), "checks": checks})
