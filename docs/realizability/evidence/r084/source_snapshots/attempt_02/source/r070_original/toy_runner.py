    ...
    for entry in Path('/proc').iterdir():
        ...
        try:
            ...
            try:
                ...
                name = 'unavailable'
            records[pid] = (int(fields[1]), int(fields[3]), int(fields[21]), name, executable)
        except (FileNotFoundError, ProcessLookupError):
            continue
    included = {pid for pid, (_, session, _, _, _) in records.items() if session == root_pid}
    included.add(root_pid)
    while True:
        expanded = included | {pid for pid, (parent, _, _, _, _) in records.items() if parent in included}
        if expanded == included:
            break
        included = expanded
    page_mib = os.sysconf('SC_PAGE_SIZE') / 1024**2
    details = [dict(pid=pid, name=records[pid][3], executable=records[pid][4],
                    rss_mib=max(0, records[pid][2]) * page_mib)
               for pid in included if pid in records]
    rss_pages = sum(max(0, records[pid][2]) for pid in included if pid in records)
    return rss_pages * page_mib, included, sorted(details, key=lambda row: row['rss_mib'], reverse=True)
...
def monitor(command, deadline, rss_limit, log_prefix, cwd=None):
    ...
    with Path(str(log_prefix) + '.stdout').open('w') as stdout, Path(str(log_prefix) + '.stderr').open('w') as stderr:
        ...
        try:
            while True:
                ...
                previous = now
                rss, pids, details = process_tree_sample(process.pid)
                samples += 1
                if rss > peak:
                    peak = rss
                    peak_processes = details
                ...
                if now >= deadline:
                    ...
                    reason = 'rss_limit'
                if reason:
                    stop_processes = details
                    terminate_tree(process, pids)
                    break
                if process.poll() is not None:
                    # A successful child must not leave a live descendant behind.
                    remaining_rss, remaining_pids, remaining_details = process_tree_sample(process.pid)
                    if remaining_rss > 0:
                        reason = 'child_left_live_descendants'
                        stop_processes = remaining_details
    ...
    return dict(command=command, returncode=process.returncode,
        ...
        per_process_at_peak=peak_processes, per_process_at_stop=stop_processes)
