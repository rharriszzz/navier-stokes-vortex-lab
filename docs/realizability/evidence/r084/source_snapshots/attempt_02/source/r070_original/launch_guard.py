...
def finalize_child(report: dict, watch: dict, child_report: dict | None) -> dict:
    """Retain the last durable checkpoint and never fabricate postlaunch zeros."""
    report["watch"] = watch
    ...
    report["child_report_sha256"] = hashlib.sha256(
        json.dumps(child_report, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    if watch.get("stop_reason"):
        report["status"] = "partial_resource_or_process_stop"
    elif child_report.get("status") == "complete":
...
def guarded_launch(output: Path, *, enabled: bool = False,
    ...
    try:
        ...
        if source_check:
            source_check()
        if not enabled:
            raise Refusal("physical execution disabled pending reviewed launch contract")
        if before_child:
            before_child()
