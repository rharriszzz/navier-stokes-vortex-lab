...
def finalize_child(report: dict, watch: dict, child_report: dict | None, *,
    ...
    if child_report is None:
        ...
        return report
    if not isinstance(watch, dict) or not isinstance(child_report, dict):
        report.update(status="partial_failed", stage="malformed_completion_schema",
                      counts={key: None for key in COUNT_KEYS})
        report["last_observed"] = {"stage": "malformed_completion_schema", "counts": None}
        return report
    stage = child_report.get("stage", "unknown_child_stage")
    raw_counts = child_report.get("counts") if isinstance(child_report, dict) else None
    ...
    accepted = (
        type(watch.get("schema")) is int and watch.get("schema") == 3 and
        type(child_report.get("schema")) is int and child_report.get("schema") == 2 and
        set(watch) == {"status", "stop_reason", "returncode", "membership_complete",
            "cleanup_confirmed", "schema", "primary_reasons", "secondary_errors",
            "workload_cleanup", "helper_cleanup", "source_manifest_sha256",
            "child_report_sha256", "cleanup_errors", "survivors"} and
        set(child_report) == {"schema", "status", "stage", "evidence_complete",
            "primary_reasons", "errors", "counts", "workload_cleanup", "helper_cleanup",
            "scientific_checks_passed", "source_manifest_sha256"} and
        ...
        watch.get("primary_reasons") == [] and watch.get("secondary_errors") == [] and
        watch.get("cleanup_errors") == [] and watch.get("survivors") == [] and
        _cleanup_ok(watch.get("workload_cleanup")) and
        _cleanup_ok(watch.get("helper_cleanup")) and
        ...
        child_report.get("primary_reasons") == [] and child_report.get("errors") == [] and
        _exact_counts(child_report.get("counts")) and
        child_report.get("counts") == {key: 0 for key in COUNT_KEYS} and
        _cleanup_ok(child_report.get("workload_cleanup")) and
        _cleanup_ok(child_report.get("helper_cleanup")) and
...
def _cleanup_ok(value) -> bool:
    if not isinstance(value, dict) or set(value) != {
            "confirmed", "status", "survivors", "errors", "identities", "requested_actions"}:
        return False
    identities = value.get("identities") if isinstance(value, dict) else None
    actions = value.get("requested_actions") if isinstance(value, dict) else None
    identities_ok = (isinstance(identities, list) and all(
        isinstance(item, (list, tuple)) and len(item) == 2 and
        all(type(part) is int and part > 0 for part in item) for item in identities))
    identities_ok = identities_ok and len({tuple(item) for item in identities}) == len(identities)
    actions_ok = (isinstance(actions, list) and bool(actions) and
                  all(isinstance(item, str) and item for item in actions))
    return (type(value.get("confirmed")) is bool and
            value.get("confirmed") is True and value.get("survivors") == [] and
            value.get("errors") == [] and
            value.get("status") == "cleanup_confirmed" and
            identities_ok and bool(identities) and actions_ok)


def _exact_counts(value) -> bool:
    return (isinstance(value, dict) and set(value) == set(COUNT_KEYS) and
