"""R018 read-only archive review and disposable source preparation; no FEM imports."""
import ast
...
def main():
    ...
    historical = subprocess.check_output(['git', 'ls-tree', '-r', '--name-only', base,
        ...
        'docs/realizability/evidence/r016', 'docs/realizability/evidence/r017'], text=True).splitlines()
    hashes = {}
    ...
    for name in ('physical.py', 'toy_runner.py', 'kernels.py', 'disk.py',
                 'wrapper_toys.py', 'block_rhs_toys.py'):
        shutil.copyfile(OLD / 'r017' / name, HERE / name)
