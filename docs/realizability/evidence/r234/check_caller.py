"""R234 caller checks: standard library only, no manager or reservation."""
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory() as other:
            os.chdir(other)
            namespace = runpy.run_path(str(CALLER), run_name='r234_caller_check')
            g = namespace['main'].__globals__
            run = g['RUN']
            assert run == Path('/tmp/navier-poiseuille-r234-once')
            assert not run.exists() and not run.is_symlink()
            assert g['INVENTORY'] == ROOT/'docs/realizability/evidence/r233/checks.json'
            ...
            altered['source_sha256']['verification/nonlinear_port/sparse.py'] = '0'*64
