...
def calculate(output):
    ...
    try:
        ...
        from dolfinx.fem.petsc import assign,apply_lifting,set_bc
        ...
        def same_mesh(request_config,size):
            ...
            reuses.append(True)
            return checked
        def compatibility(vector,nulls):
            before=vector.copy()
            dots=[float(n.dot(before)) for n in nulls]
            for n in nulls:
                vector.axpy(-n.dot(vector),n)
            removed=before.copy()
            removed.axpy(-1,vector)
            data=dict(pressure_constant_products_before=dots,removed_norm=float(removed.norm()),
                rhs_norm=float(before.norm()),tolerance=256*np.finfo(float).eps*float(before.norm()))
        ...
        def capture(*args,**kwargs):
            ...
            def trace(frame,event,arg):
                if frame.f_code is original.__code__ and event=='line' and frame.f_lineno==projection_line:
                    ...
                    record['P_pressure_compatibility']=compatibility(copy,local['null_vectors'])
                return trace
            record['primary_rhs']+=1
            checkpoint('P_assembly_and_primary_solve')
            sys.settrace(trace)
            try:
        ...
        bcs=captured['bcs']
        constrained=np.concatenate([bc.dof_indices()[0]+offsets[i]
            for i in (0,2) for bc in bcs if bc.function_space==spaces[i]])
        def matrix_digest():
        ...
        def counts():
            return {n:int(PETSc.Log.Event(n).getPerfInfo()['count'])
                    for n in ('MatLUFactorSym','MatLUFactorNum','MatSolve')}
        ...
        for name,(projection,load) in loads.items():
            ...
            new_constrained=np.concatenate([bc.dof_indices()[0]+offsets[i]
                for i in (0,2) for bc in new_bcs if bc.function_space==spaces[i]])
            assert np.array_equal(new_constrained,constrained)
            rhs=b.copy()
            rhs.set(0.)
            rhs.array[offsets[0]:offsets[1]]=load.real
            rhs.array[offsets[2]:offsets[3]]=load.imag
            apply_lifting(rhs,captured['a'],bcs=fem.bcs_by_block(fem.extract_function_spaces(captured['a'],1),new_bcs))
            rhs.ghostUpdate(addv=PETSc.InsertMode.ADD,mode=PETSc.ScatterMode.REVERSE)
            set_bc(rhs,fem.bcs_by_block(spaces,new_bcs))
            record['loads'][name]['pressure_compatibility']=compatibility(rhs,nulls)
            checkpoint(name+'_compatible_primary_rhs')
            ...
            for index,normal in ((0,normal_real),(2,normal_imag)):
                ...
                dofs=np.concatenate([bc.dof_indices()[0] for bc in new_bcs if bc.function_space==spaces[index]])
