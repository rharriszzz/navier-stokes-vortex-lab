...
def toys():
    ...
    budget_path = HERE / 'toy_budget.json'
    prior = support.read_json(budget_path)['prior_elapsed_seconds'] if budget_path.exists() else 0.0
    deadline = started + 600.0 - prior
    record = dict(status='partial', wall_limit_seconds=600.0, rss_limit_mib=1536.0,
                  physical_meshes=0, matrix_solves=0, watches={}, prior_elapsed_seconds=prior)
    phases = [
    ...
    wrong_root_watch = support.monitor(
        ...
        deadline, 1536.0, output / 'wrong-root', cwd=wrong_root_cwd)
    record['watches']['wrong_root'] = wrong_root_watch
    record['current_attempt_seconds'] = time.monotonic() - started
    record['elapsed_seconds'] = prior + record['current_attempt_seconds']
    ...
    for name, filename, flag, report_name in phases:
        ...
        watch = support.monitor(command, deadline, 1536.0, output / name)
