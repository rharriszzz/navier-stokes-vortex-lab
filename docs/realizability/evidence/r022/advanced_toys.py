...
def run(output):
    ...
    try:
        ...
        oracle_matrix = assemble_matrix(fem.form(forms))
        ...
        for index in (0, 2):
            oracle_load[offsets[index]:offsets[index+1]] = fem.assemble_vector(fem.form(rhs_forms[index])).array
